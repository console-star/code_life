let requestAnimFrame =
	...
	vueProgress = function(options) {
		let endAngleRad = Math.PI / 360 * 230
		...
		this._radius = options.radius || 40 //circle的半径
		...
		this._colors = options.pathColors || ['#EEE', '#f35e3e'] // path的fill颜色
		...
		this._wrpClass = options.wrapClass || 'circles-wrap1'
		this._textClass = options.textClass || 'circles-text1'
...
vueProgress.prototype = {
	...
	htmlifyNumber: function(number, integerPartClass, decimalPartClass) {
		integerPartClass = integerPartClass || 'circles-integer1'
		decimalPartClass = decimalPartClass || 'circles-decimals1'

		var parts = (number + '').split('.'),
			html = '<p class="' + integerPartClass + '">' +number+ '</p>'
